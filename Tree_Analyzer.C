#include "function_defination_BalFunc.h" //call_libraries.h, input_variables.h and histogram_definition_BalFunc.h are called within function_defination_BalFunc.h
...
void Tree_Analyzer(TString input_file, int itxtoutFile, TString out_file, TString colliding_system, int isMC)
{
  ...
  for(int i = 0; i < nevents; i++) //event loop start
      ...
      for (int j = 0; j < nref; j++) //Jet loop start
	{
	  ...
	  find_leading_subleading_Jets(jet_pt_corr, jt_eta, jt_eta, ld_CorrpT, ld_CorrEta, ld_CorrPhi, sld_CorrpT, sld_CorrEta, sld_CorrPhi);
	  ...
	  for(int irctrk = 0; irctrk < ntrk; irctrk++)
	      ...
	      if(colliding_system=="PbPb")
		{
		  ...
		  if(trk_pt < trk_minpt_cut) continue;
	      ...
	      else if(colliding_system=="pp")
		{
	      ...
	      //halltrk_pt->Fill(trk_pt);
	      //halltrk_eta->Fill(trk_eta);
	      //halltrk_phi->Fill(trk_phi);
	      ...
	      if(nrfgn == 1)
		{
		  if(index_ref >= 0 && index_gen >= 0)
			...
			{                                                                                                              
			  ...
			  if(gentrk_pt < trk_minpt_cut) continue;
			  if(fabs(gentrk_eta) > trk_eta_cut) continue;

			  //halltrk_Gen_pt->Fill(gentrk_pt);
			  //halltrk_Gen_eta->Fill(gentrk_eta);
			  //halltrk_Gen_phi->Fill(gentrk_phi);
  ...
  if(is_MC)
    {
      ...
      //for gen
      mixing_corr(Evtw_vec_1D, hiBin_vec_1D, jet_vec_2D_gn, Matched_refpartonB_Vec_gn, jetw_Vec_2D_gn, jet_oldtrk_vec_3D_gn, jet_newtrk_vec_3D_gn, jet_oldchtrk_vec_3D_gn, jet_newchtrk_vec_3D_gn, jet_trk_charge_3D_gn, vertexz_vec_1D, false);
    }

  std::cout<<endl;
  std::cout<<"~~~~~~~Mixing correlation for Reco/data level~~~~~~~~~~"<<std::endl;
  // for reco and data
  mixing_corr(Evtw_vec_1D, hiBin_vec_1D, jet_vec_2D_rc, Matched_refpartonB_Vec_rc, jetw_Vec_2D_rc, jet_oldtrk_vec_3D_rc, jet_newtrk_vec_3D_rc, jet_oldchtrk_vec_3D_rc, jet_newchtrk_vec_3D_rc, jet_trk_charge_3D_rc, vertexz_vec_1D, true);
